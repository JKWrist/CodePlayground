...
class TelnetClient():
    ...
    def check_internet(self):
        if False == self.check_ping_domain():
            if False == self.check_dns_state():
                if False == self.check_dhcpc_state():
                    if False == self.check_dhcp_state():
                        return False
        else:
             return True

    def check_ping_domain(self):
        command = 'ping baidu.com -c'
        # 执行命令
        self.tn.write(command.encode('ascii') + b'\n')
        time.sleep(4)
        # 获取命令结果
        command_result = self.tn.read_very_eager().decode('ascii')
        logging.warning('命令执行结果：\n%s' % command_result)
        ele = "5 packets transmitted, 5 packets received, 0% packet loss"
        if ele in command_result:
            print("可以ping通域名")
            return True
        else:
    ...
    def check_ping_ip(self):
        command = 'ping 114.114.114.114 -c'
        # 执行命令
        self.tn.write(command.encode('ascii') + b'\n')
        time.sleep(4)
        # 获取命令结果
...
class Test():
    def test1(self, ):
        for i in range(5000):
            ...
            web = Webdriver(url="http://192.168.11.1")
            web.dut_login("admin", "system")
            web.dut_opentelnet()
            web.dut_rebootwifi()
            telent = TelnetClient()
            telent.login_host("192.168.11.1", "admin", "system")
    ...
    def test_internet(self):
        for i in range(5000):
            ...
            time.sleep(25)
            telent = TelnetClient()
            telent.login_host("cmcc.wifi 2356", "admin", "system")
